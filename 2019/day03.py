...
def stepsToCoord(coord, route):
    for steps, dest in enumerate(route):
        if coord == dest:
            return steps

    print (f"Uh oh something went wrong, coord {coord} not found :(")
    exit()
...
wire1route = createRoute(wire1)
# print(f"Wire 1 route: {wire1route}\n")

wire2route = createRoute(wire2)
# print(f"Wire 2 route: {wire2route}\n")

overlap = set(wire1route).intersection(wire2route)
overlap.remove((0,0))
print(f"Overlap is: {overlap}\n")

for intersection in overlap:
    distance = abs(intersection[0]) + abs(intersection[1])
    if min_abs_distance is None or min_abs_distance > distance:
        min_abs_distance = distance

    w1_steps = stepsToCoord(intersection, wire1route)
    w2_steps = stepsToCoord(intersection, wire2route)
    if min_steps is None or min_steps > w1_steps + w2_steps:
        min_steps = w1_steps + w2_steps

print(f"Minimum distance: {min_abs_distance}")
